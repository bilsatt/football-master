...
def calc_goal_probs(lam_h: float, lam_a: float):
    p_home = 1 - np.exp(-lam_h)
    p_away = 1 - np.exp(-lam_a)
    p_btts = 1 - np.exp(-lam_h) - np.exp(-lam_a) + np.exp(-(lam_h + lam_a))
    p_over05 = 1 - np.exp(-(lam_h + lam_a))
    return p_home, p_away, p_btts, p_over05

def _poisson_cdf(k: int, lam: float) -> float:
    i = np.arange(0, k+1)
    pmf = np.exp(-lam) * (lam ** i) / np.vectorize(np.math.factorial)(i)
    return float(pmf.sum())

def calc_ou_probs(lam_h: float, lam_a: float):
    lam_t = lam_h + lam_a
    p_over15  = 1.0 - _poisson_cdf(1, lam_t)
    p_under15 = _poisson_cdf(1, lam_t)
    p_over25  = 1.0 - _poisson_cdf(2, lam_t)
    p_under25 = _poisson_cdf(2, lam_t)
    return {"Over1.5": p_over15, "Under1.5": p_under15, "Over2.5": p_over25, "Under2.5": p_under25, "Lambda_Total": lam_t}
...
with panes[0]:
    ...
    if (home_id and away_id) and st.button("Analiz & Tahmin", key="go_single"):
        ...
        with st.expander(f"{home_team} • Son 5"): st.dataframe(hf_df if not hf_df.empty else pd.DataFrame([{"📄":"Veri bulunamadı"}]))  # noqa: E501
        with st.expander(f"{away_team} • Son 5"): st.dataframe(af_df if not af_df.empty else pd.DataFrame([{"📄":"Veri bulunamadı"}]))
        ...
        st.subheader("📐 Dixon–Coles Tahmin + Guardrails + O/U")
        if not MODEL_OK:
            ...
            ou = calc_ou_probs(lam_h, lam_a)
            st.subheader("⚖️ Over/Under Olasılıkları")
            c5, c6, c7, c8 = st.columns(4)
            c5.metric("Over 1.5",  f"{ou['Over1.5']:.2f}")
            c6.metric("Under 1.5", f"{ou['Under1.5']:.2f}")
            c7.metric("Over 2.5",  f"{ou['Over2.5']:.2f}")
            c8.metric("Under 2.5", f"{ou['Under2.5']:.2f}")
            st.caption(f"Toplam beklenen gol (λ_total) ≈ {ou['Lambda_Total']:.2f}")

            suggestion = None
            if ou["Over1.5"] >= 0.65:
                suggestion = f"Öneri: **Over 1.5** (p≈{ou['Over1.5']:.2f})"
            elif ou["Under2.5"] >= 0.60:
                suggestion = f"Öneri: **Under 2.5** (p≈{ou['Under2.5']:.2f})"
            if suggestion:
                st.info(suggestion)
            ...
            with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
                ...
                model_df = pd.DataFrame({
                    ...
                    "P_Over1_5":[ou["Over1.5"]], "P_Under1_5":[ou["Under1.5"]],
                    "P_Over2_5":[ou["Over2.5"]], "P_Under2_5":[ou["Under2.5"]],
                    "Lambda_Total":[ou["Lambda_Total"]],
            ...
            st.download_button("Excel'i İndir (Tek Maç)", data=buf.getvalue(),
                               file_name=f"master_single_v5d_{home_team}_vs_{away_team}_{season}.xlsx",
